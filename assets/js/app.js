...
Customers:['ID','ShopName','CustomerName','CustomerType','Phone','Area','GSTIN','CreditLimit','Address','Active','Notes'],
Suppliers:['ID','CompanyName','ContactName','Phone','Email','Area','GSTIN','Address'],
Sales:['ID','Invoice','CustomerID','Customer','CustomerType','ProductID','ProductClass','Product','Category','Size','Quantity','UnitPrice','Discount','Total','Paid','Due','PaymentMethod','RecordedBy','Date'],
Purchases:['ID','Invoice','SupplierID','Supplier','ProductID','ProductClass','Product','Category','Size','Quantity','UnitCost','Total','Paid','Due','PaymentMethod','RecordedBy','Date'],
'Stock Movements':['ID','ProductID','ProductClass','Product','Category','Size','Type','Quantity','ReferenceID','Date'],
...
async function buildWorkbook(){
 ...
 const c=db.customers.map(x=>({ID:x.id,ShopName:x.shop,CustomerName:x.name,CustomerType:x.type||'Walk-in Customer',Phone:x.phone,Area:x.area,GSTIN:x.gstin,CreditLimit:x.credit,Address:x.address,Active:x.active!==false,Notes:x.notes||''}));
 ...
 const sales=db.sales.map(x=>({ID:x.id,Invoice:x.invoice,CustomerID:x.customerId,Customer:x.customer,CustomerType:x.customerType,ProductID:x.productId,ProductClass:x.productClass,Product:x.product,Category:x.category,Size:x.size,Quantity:x.qty,UnitPrice:x.price,Discount:x.discount,Total:x.total,Paid:x.paid,Due:x.due,PaymentMethod:x.payment,RecordedBy:x.recordedBy,Date:x.date}));
 const purchases=db.purchases.map(x=>({ID:x.id,Invoice:x.invoice,SupplierID:x.supplierId,Supplier:x.supplier,ProductID:x.productId,ProductClass:x.productClass,Product:x.product,Category:x.category,Size:x.size,Quantity:x.qty,UnitCost:x.cost,Total:x.total,Paid:x.paid,Due:x.due,PaymentMethod:x.payment,RecordedBy:x.recordedBy,Date:x.date}));
 setSheet(wb,'Subproducts',sp,headers.Subproducts);setSheet(wb,'Products_Inventory',p,headers.Products);setSheet(wb,'Customers',c,headers.Customers);setSheet(wb,'Suppliers',s,headers.Suppliers);setSheet(wb,'Sales',sales,headers.Sales);setSheet(wb,'Purchases',purchases,headers.Purchases);
 const mov=[...db.sales.map(x=>({ID:uid(),ProductID:x.productId,ProductClass:x.productClass,Product:x.product,Category:x.category,Size:x.size,Type:'SALE',Quantity:-x.qty,ReferenceID:x.invoice,Date:x.date})),...db.purchases.map(x=>({ID:uid(),ProductID:x.productId,ProductClass:x.productClass,Product:x.product,Category:x.category,Size:x.size,Type:'PURCHASE',Quantity:x.qty,ReferenceID:x.invoice,Date:x.date}))];
...
function loadData(wb){
 sourceWorkbook=wb;
 db.lists={};const lr=read(wb,'Lists');if(lr.length){const keys=Object.keys(lr[0]);keys.forEach(k=>{db.lists[k]=lr.map(r=>r[k]).filter(v=>v!==''&&v!=null).map(String)})}
 db.customerTypes=read(wb,'Customer_Types').map(r=>r.CustomerType||r.Type||'').filter(Boolean);
 ...
 db.customers=read(wb,'Customers').map(r=>({id:r.ID||uid(),shop:r.ShopName||'',name:r.CustomerName||r.Name||'',type:r.CustomerType||'Walk-in Customer',phone:r.Phone||r.ContactNumber||'',area:r.Area||'',gstin:r.GSTIN||'',credit:+(r.CreditLimit||0),address:r.Address||'',active:String(r.Active).toLowerCase()!=='false',notes:r.Notes||''}));
...
function render(){
 ...
 $('lowStockCount').textContent=[...db.products,...db.subproducts].filter(p=>p.stock<=p.reorder).length;
 $('saleCustomer').innerHTML='<option value="">Walk-in customer</option>'+db.customers.filter(x=>x.active!==false).map(x=>'<option value="'+esc(x.id)+'">'+esc(x.name||x.shop||'Unnamed')+' — '+esc(x.type||'Walk-in Customer')+'</option>').join('');$('saleProduct').innerHTML=opts(db.products,'Choose paint / main product');$('saleSubproductCategory').innerHTML='<option value="">Choose category</option>'+[...new Set(db.subproducts.filter(x=>x.active!==false).map(x=>x.category))].sort().map(x=>'<option value="'+esc(x)+'">'+esc(x)+'</option>').join('');$('saleSubproductSize').innerHTML='<option value="">Choose size</option>';$('saleSubproduct').innerHTML='<option value="">Choose subproduct</option>';$('purchaseSupplier').innerHTML=opts(db.suppliers,'Choose supplier');$('purchaseProduct').innerHTML=opts(db.products,'Choose product');$('purchaseSubproductCategory').innerHTML='<option value="">Choose category</option>'+[...new Set(db.subproducts.filter(x=>x.active!==false).map(x=>x.category))].sort().map(x=>'<option value="'+esc(x)+'">'+esc(x)+'</option>').join('');$('purchaseSubproductSize').innerHTML='<option value="">Choose size</option>';$('purchaseSubproduct').innerHTML='<option value="">Choose subproduct</option>';if($('cType'))$('cType').innerHTML='<option value="">Choose customer type</option>'+(db.customerTypes.length?db.customerTypes:['Walk-in Customer','Painter','Contractor','Builder / Developer','Interior Designer / Architect','Commercial / Institutional','Hardware / Paint Dealer','Carpenter / Fabricator','Property / Facility Manager','Other']).map(x=>'<option value="'+esc(x)+'">'+esc(x)+'</option>').join('');
...
function updateSubSizes(prefix){const cat=$('prefixSubproductCategory'.replace('prefix',prefix)).value;const rows=db.subproducts.filter(x=>x.active!==false&&x.category===cat);const sizes=[...new Set(rows.map(x=>String(x.size)).filter(Boolean))];$(''+prefix+'SubproductSize').innerHTML='<option value="">Choose size</option>'+sizes.map(s=>{const r=rows.find(x=>String(x.size)===s);return '<option value="'+esc(s)+'">'+esc(s+' '+(r?.unit||''))+'</option>'}).join('');$(''+prefix+'Subproduct').innerHTML='<option value="">Choose subproduct</option>'}
function updateSubs(prefix){const cat=$(''+prefix+'SubproductCategory').value,size=$(''+prefix+'SubproductSize').value;const rows=db.subproducts.filter(x=>x.active!==false&&(!cat||x.category===cat)&&(!size||String(x.size)===size));$(''+prefix+'Subproduct').innerHTML='<option value="">Choose subproduct</option>'+rows.map(x=>'<option value="'+esc(x.id)+'">'+esc((x.sku||'NO-SKU')+' — '+x.name+(x.variant?' — '+x.variant:'')+' — '+x.size+' '+x.unit+' — Stock '+x.stock)+'</option>').join('')}
function bind(){
...
$('customerForm').addEventListener('submit',async e=>{e.preventDefault();const x={id:uid(),shop:$('cShop').value.trim(),name:$('cName').value.trim(),type:$('cType').value||'Walk-in Customer',phone:$('cPhone').value.trim(),area:$('cArea').value.trim(),gstin:$('cGstin').value.trim(),credit:+$('cCredit').value,address:$('cAddress').value.trim(),active:true,notes:$('cNotes').value.trim()};if(!x.name)return toast('Customer name is required.',true);db.customers.push(x);try{await writeExcel();$('customerDialog').close();render();toast('Customer saved and synced.')}catch(err){db.customers.pop();toast(err.message,true)}});
...
$('saleSubproductCategory').addEventListener('change',()=>{updateSubSizes('sale');$('saleProduct').value=''});$('saleSubproductSize').addEventListener('change',()=>updateSubs('sale'));$('saleSubproduct').addEventListener('change',()=>{if($('saleSubproduct').value)$('saleProduct').value=''});$('saleProduct').addEventListener('change',()=>{if($('saleProduct').value){$('saleSubproductCategory').value='';$('saleSubproductSize').innerHTML='<option value="">Choose size</option>';$('saleSubproduct').innerHTML='<option value="">Choose subproduct</option>'}});
$('purchaseSubproductCategory').addEventListener('change',()=>{updateSubSizes('purchase');$('purchaseProduct').value=''});$('purchaseSubproductSize').addEventListener('change',()=>updateSubs('purchase'));$('purchaseSubproduct').addEventListener('change',()=>{if($('purchaseSubproduct').value)$('purchaseProduct').value=''});$('purchaseProduct').addEventListener('change',()=>{if($('purchaseProduct').value){$('purchaseSubproductCategory').value='';$('purchaseSubproductSize').innerHTML='<option value="">Choose size</option>';$('purchaseSubproduct').innerHTML='<option value="">Choose subproduct</option>'}});
$('saleForm').addEventListener('submit',async e=>{e.preventDefault();const main=db.products.find(x=>x.id===$('saleProduct').value),subp=db.subproducts.find(x=>x.id===$('saleSubproduct').value),p=subp||main,q=+$('saleQty').value;if(!p||q<=0)return toast('Choose a main product or subproduct and quantity.',true);if(p.stock<q)return toast('Insufficient stock. Available: '+p.stock,true);const c=db.customers.find(x=>x.id===$('saleCustomer').value),discount=Math.max(0,+$('saleDiscount').value||0),total=Math.max(0,q*p.sell-discount),paid=Math.max(0,+$('salePaid').value||0);if(paid>total)return toast('Paid amount cannot exceed total.',true);const x={id:uid(),invoice:'SAL-'+Date.now(),customerId:c?.id||'',customer:c?.name||'Walk-in customer',customerType:c?.type||'Walk-in Customer',productId:p.id,productClass:subp?'Subproduct':'Main Product',product:p.name,category:subp?p.category:'',size:subp?p.size:p.packSize,qty:q,price:p.sell,discount,total,paid,due:total-paid,payment:$('salePayment').value,recordedBy:$('saleRecordedBy').value,date:new Date().toISOString()};p.stock-=q;db.sales.push(x);try{await writeExcel();e.target.reset();render();toast(x.invoice+' saved and synced.')}catch(err){p.stock+=q;db.sales.pop();toast(err.message,true)}});
$('purchaseForm').addEventListener('submit',async e=>{e.preventDefault();const main=db.products.find(x=>x.id===$('purchaseProduct').value),subp=db.subproducts.find(x=>x.id===$('purchaseSubproduct').value),p=subp||main,q=+$('purchaseQty').value,cost=Math.max(0,+$('purchaseCost').value||0);if(!p||q<=0)return toast('Choose a main product or subproduct and quantity.',true);const s=db.suppliers.find(x=>x.id===$('purchaseSupplier').value),total=q*cost,paid=Math.max(0,+$('purchasePaid').value||0);if(paid>total)return toast('Paid amount cannot exceed total.',true);const x={id:uid(),invoice:$('purchaseInvoice').value.trim()||'PUR-'+Date.now(),supplierId:s?.id||'',supplier:s?.name||'Unknown supplier',productId:p.id,productClass:subp?'Subproduct':'Main Product',product:p.name,category:subp?p.category:'',size:subp?p.size:p.packSize,qty:q,cost,total,paid,due:total-paid,payment:$('purchasePayment').value,recordedBy:$('purchaseRecordedBy').value,date:new Date().toISOString()};const oldCost=p.cost;p.stock+=q;p.cost=cost;db.purchases.push(x);try{await writeExcel();e.target.reset();render();toast(x.invoice+' saved and synced.')}catch(err){p.stock-=q;p.cost=oldCost;db.purchases.pop();toast(err.message,true)}});
